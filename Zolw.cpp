...
Zolw::Zolw(int y, int x, Swiat* swiat) {
	this->setY(y);
	this->setX(x);
	this->setSila(2);
...
void Zolw::kolizja(int opp_y, int opp_x, Zwierze* napastnik) {
	if (this->getGatunek() == napastnik->getGatunek()) {
		...
		if (napastnik->getSila() < 5) {
			this->getSwiat()->setEvents(this->getSwiat()->getEvents() + this->getGatunek() + " odpiera atak " + napastnik->getGatunek() + "\n");
			return;
		}
		else {
